...
const AUDITED_PATH_PATTERNS = [/\/api\/entries/, /\/api\/pending/, /\/api\/policy/, /\/api\/collections/, /\/api\/apis/];
...
export const auditInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  const resource = urlParts.find(p => p === 'entries' || p === 'pending' || p === 'policy' || p === 'collections' || p === 'apis') ?? 'unknown';
